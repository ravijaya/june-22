...
sales = pd.read_excel("sample-salesv2.xlsx", parse_dates=['date'])
...
bar_plot = sales_totals.sort_values(by='ext price', ascending=False).\
...
stack_bar_plot = category_group.unstack().\
    plot(kind='bar',stacked=True, title="Total Sales by Customer", figsize=(9, 7))
...
month_plot = purchase_patterns.resample('M').sum()\
    .plot(title="Total Sales by Month",legend=None)
